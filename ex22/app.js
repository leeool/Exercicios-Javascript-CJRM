...
characters.sort((item1, item2) => item1.id - item2.id)
...
const firstNumberGreatherThan50 = randomNumbers.find(number => number > 50)

console.log(firstNumberGreatherThan50)
...
const peopleInAlphabeticalOrderReverse = people
  .map(item => item)
  .sort()
  .reverse()

console.log(peopleInAlphabeticalOrderReverse)
...
const ingredients = ['vinho', 'tomate', 'abobrinha', 'cogumelo', 'cebola']

const cookedFoods = ingredients.reduce((acc, item, index, array) => {
...
const dogsInHumanAge = pets.filter(({ type }) => type === "Dog")
