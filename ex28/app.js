...
myInfo.incrementAge = () => myInfo.age++
...
myInfo.walk = (meters) => {
  ...
  meters.forEach((meter) => myInfo.metersWalked += meter)
}

myInfo.walk([10, 12, 15, 120])
...
const getCorrectWord = (logicalTest, a, b) => {
  return logicalTest ? a : b
}

myInfo.introduction = () => {
  const { gender, age, metersWalked, height, name, weight } = myInfo

  const correctPronoum =
    getCorrectWord(gender === "Feminino", "a", "o")

  const pluralOrSingularAge =
    getCorrectWord(age === 1, "ano", "anos")

  const pluralOrSingularMeters =
    getCorrectWord(metersWalked === 1, "metro", "metros")

  const pluralOrSingularHeight =
    getCorrectWord(height === 1, "metro", "metros")

  return `Oi. Eu sou ${correctPronoum} ${name}, tenho ${age} ${pluralOrSingularAge}, ${height} ${pluralOrSingularHeight} de altura, peso ${weight} quilos e, só hoje, eu já caminhei ${metersWalked} ${pluralOrSingularMeters}.`
}

console.log(myInfo.introduction())
...
const isTruthyOrFalsy = (item) => {
  item.forEach(item => {
    console.log(Boolean(item))
  })
}

isTruthyOrFalsy([undefined, null, 0, "", NaN, false, -0])
isTruthyOrFalsy([" ", 1, () => { }, true, [], {}, -10])
...
const getBook = (bookName) => {
  const bookList = {
    "A Sutil arte de ligar o foda-se": {
      pageQuantity: 60,
      author: "Jurema",
      publisher: "Jurema Entertainment"
    },
    "Pai Rico, Pai Pobre": {
      pageQuantity: 105,
    ...
    "Revolução dos Bixos": {
      pageQuantity: 54,
      author: "Olavo",
      publisher: "Liberalismo Brasil"
    }
  }


  return bookList[bookName] || bookList
}

console.log(getBook("Revolução dos Bixos"))
