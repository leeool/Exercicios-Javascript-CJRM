...
const logUserInfo = async (username) => {
  console.log(await getUserInfo(username))
}
...
const numbersTwoAndThree = numbers.filter(
  (number) => number % 2 === 0 || number % 3 === 0
)

// console.log(numbersTwoAndThree)
...
const name = ["N", "a", , "t", "á", "l", "i", "a"]
const regex = /^[aeiou]$/

name.forEach((letter, index) => {
  regex.test(letter) ? (name[index - 1] += "P") : letter
})
...
const myNameSepareted = myName.split("")

myNameSepareted.forEach((letter, index) => {
...
const getItemOccurrence = (array, wantedItem) => {
  return array.reduce((acc, item) => {
    return item === wantedItem ? acc + 1 : acc
  }, 0)
}

// const getItemOccurrence = (array, wantedItem) => {
//   array.forEach((item) => {
//     item === wantedItem ? i++ : null
//   })

//   return i
// }

console.log(getItemOccurrence(scores, 100))
...
const filter = (array, func) => {
  ...
  array.forEach((item, index, array) => {
    func(item, index, array) ? newArray.push(item) : null
  })

  return newArray
}

console.log(filter([1, 2, 3, 5, 2], (item, index, array) => index === array.indexOf(item)))
