...
const logRequest = (error, data) => error
  ? console.log(error)
  : console.log(data)

const getPokemonURL = pokemon => `https://pokeapi.co/api/v2/pokemon/${pokemon}`

getRequest(getPokemonURL("bulbasaur"), (error, data) => {
  console.log("callback executado")
  logRequest(error, data)
  getRequest(getPokemonURL("charmander"), (error, data) => {
    logRequest(error, data)
    getRequest(getPokemonURL("squirtle"), (error, data) => {
      logRequest(error, data)
...
const convertToHex = color => {
  ...
  return colors[colorLowerCase]
    ? `O hexadecimal para a cor ${colorLowerCase} é ${colors[colorLowerCase]}`
    : `Não temos o equivalente hexadecimal para ${colorLowerCase}`

}

const colors = ["pink", "yellow", "brown", "red", "blue", "black", "green", "cian"]
