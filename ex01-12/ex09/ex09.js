...
const caracCounter = (stringFunc) => stringFunc.length;

console.log(caracCounter("olá"));
...
const characterLocation = (charac, string) => {
  return string.indexOf(charac);
};

console.log(characterLocation("o", "leonardo gonsalez"));
...
console.log(isInArray(["joao", "miguel", "paulo", "pedro"], "pedro"));
...
console.log(concatArray(["leo", "gus", "pamela"], ["pedro", "gui", "paulo"]));
...
console.log(removeLastItem(["Hulk", "Superman", "Cat Woman", "Batman"]));
...
// const numbersInfo = (number, index, array) => {
...
const postLikes = (peoplesArray = []) => {
  const onePeopleLike = `${peoplesArray.join(", ")} curtiu isso`
  const twoPeopleLike = `${peoplesArray.join(" e ")} curtiram isso`
  const threePeopleLike = `${peoplesArray.join(", ")} curtiram isso`



  switch (peoplesArray.length) {
    case 0:
      return console.log("Ninguem curtiu isso");
    case 1:
      return console.log(onePeopleLike);
    case 2:
      return console.log(twoPeopleLike);
    case 3:
      return console.log(threePeopleLike);
    default:
      for(let i = 0; i < peoplesArray.length; i++){
        peopleCounter++
      }
      const morePeopleLike = `${peoplesArray[0]}, ${peoplesArray[1]} e mais ${peopleCounter - 2} pessoas curtiram isso`
      return console.log(morePeopleLike)
  }
    
};

postLikes(["maria", "paulo", "pedro", "joao"]);
