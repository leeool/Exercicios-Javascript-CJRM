...
const fruits = ["pera", "banana", "mamão"];
...
const isMorning = horario >= 6 && horario < 12
const isAfternoon = horario >= 12 && horario < 18
const isNight = horario >= 18 || horario >= 0 && horario < 6
...
if(age <= 7 || age >= 65){
  ...
  console.log("A entrada é de R$ 30,00")
}
...
//     xBooleans.push(crazyArray[i])
//   }else if(typeof crazyArray[i] == "number"){
//     xNumbers.push(crazyArray[i])
//   }else if(typeof crazyArray[i] == "string"){
//     xStrings.push(crazyArray[i])
//   }
// }

// console.log(` crazyArray tem ${xBooleans.length} booleans, ${xNumbers.length} números e ${xStrings.length} strings.`)

// if(crazyArray.includes(false) || crazyArray.includes(true)){
//   for(let i = 0; i )
// }
...
for(let i = 0; i < randomNumbers.length; i++){
  if(randomNumbers[i] % 2 == 0){
    numberEven.push(randomNumbers[i])
  }else if(randomNumbers[i] % 2 != 0){
    numberOdd.push(randomNumbers[i])
  }
}

console.log(`Numeros ímpares: ${numberOdd.join(", ").replace(", 3", " e 3")}. Números pares: ${numberEven.join(", ").replace(", 5", " e 5")}.`)
