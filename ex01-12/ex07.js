...
if (animals.includes("leão") !== true) {
  console.log("Leão nao existe na array animals");
} else {
...
for (let a = 0; a < sentence.length; a++) {
  if (sentence[a] === "certeza") {
    continue;
  }

  concatString.push(sentence[a]);
}
...
for (let i = 0; i < randomValues.length; i++) {
  const isArray = typeof randomValues[i] === "string";
  const isBoolean = typeof randomValues[i] === "boolean";
  ...
  if (isArray) {
    firstStrings.push(randomValues[i]);
    stringsAmount++;
  }
...
console.log(`As quatro primeiras strings são: ${firstStrings.join(" ")}`);
console.log(
  `Até que as quatro  primeiras strings fossem iteradas, ${booleansAmount} foram iterados`
);
console.log(`O array foi iterado ${iterationsAmount} vezes`);
...
switch (drinkType) {
  ...
  case "refrigerante":
    ...
    break;
  case "suco":
    drinkDesc = "Bebida produzida do líquido extraído de frutos";
    break;
  default:
    drinkDesc = "Bebida desconhecida";
}
